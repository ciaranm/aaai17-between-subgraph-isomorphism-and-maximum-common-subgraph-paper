...
namespace
{
    ...
    struct SIP
    {
        ...
        struct Assignments
        {
            vector<tuple<unsigned, unsigned, int, bool> > trail;

            auto push_branch(unsigned a, unsigned b, unsigned p) -> void
            {
                trail.emplace_back(a, b, p, true);
            }
            ...
            auto push_implication(unsigned a, unsigned b) -> void
            {
                if (trail.end() == find_if(trail.begin(), trail.end(), [&] (const auto & x) {
                    ...
                    trail.emplace_back(a, b, -1, false);
            }

            auto pop() -> void
            {
                while ((! trail.empty()) && (! get<3>(trail.back())))
                    trail.pop_back();
            ...
            auto store_to(map<int, int> & m, unsigned wildcard_start, vector<int> & position) const -> void
            {
                for (auto & t : trail) {
        ...
        auto solve(
                ...
                atomic<unsigned long long> & nodes,
                unsigned depth) -> void
        {
            ...
            if (domains.end() == branch_domain) {
                std::unique_lock<std::mutex> lock(isomorphism_mutex);
                if (! success) {
                    success = true;
                    assignments.store_to(result.isomorphism, wildcard_start, result.position);
                }
                return;
            }
            ...
            atomic<unsigned> shared_branch_value_pos{ 0 };
            unsigned branch_value_pos_end = branch_values.size();

            auto loop_function = [&] {
                while (true) {
                    auto my_branch_value_pos = shared_branch_value_pos++;
                    if (my_branch_value_pos >= branch_value_pos_end)
                        break;

                    unsigned branch_value = branch_values[my_branch_value_pos];

                    if (! (*params.abort || success || (branch_value >= wildcard_start && my_branch_value_pos > first_wildcard_pos))) {
                        auto new_assignments = assignments;
                        new_assignments.push_branch(branch_domain->v, branch_value, my_branch_value_pos);
                        ...
                        if (unit_propagate(new_domains, new_assignments))
                            solve(new_domains, new_assignments, nodes, depth + 1);
                    }
                }
            };

            run_with_help(loop_function, depth);
        }
        ...
        auto run()
        {
            ...
            if (unit_propagate(initial_domains, assignments)) {
                atomic<unsigned long long> nodes(0);
                solve(initial_domains, assignments, nodes, 0);
                result.nodes = nodes;
            }
        }
    };
