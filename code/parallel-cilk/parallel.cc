...
namespace
{
    ...
    struct SIP
    {
        ...
        struct Assignments
        {
            ...
            auto store_to(map<int, int> & m, unsigned wildcard_start) const -> void
            {
                for (auto & t : trail) {
        ...
        auto solve(
                ...
                reducer_opadd<unsigned long long> & nodes) -> void
        {
        ...
        auto run()
        {
            ...
            if (unit_propagate(initial_domains, assignments)) {
                reducer_opadd<unsigned long long> nodes(0);
                solve(initial_domains, assignments, nodes);
