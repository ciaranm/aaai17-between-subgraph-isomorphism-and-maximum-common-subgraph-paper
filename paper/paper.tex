...
length three: removing a single vertex can delete arbitrarily many such paths. We could count paths
of any length which are vertex disjoint, although calculating this is too expensive to be useful in
practice.

To allow for fast propagation, rather than calculating paths dynamically, we follow the approach of
\citet{DBLP:conf/cp/McCreeshP15} and construct supplemental graphs. We use paths of length 2,
looking at counts of 1, 2, and 3 in the target graph (and so counts of $1 + k$ up to $3 + k$ in the
pattern).  We then investigate whether this leads to new constraints being generated.
