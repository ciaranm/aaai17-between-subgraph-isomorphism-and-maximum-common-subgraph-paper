...
\begin{figure}
    \begin{center}
        \begin{tikzpicture}[every node/.style={circle, inner sep=0.5mm,node distance=10mm,fill=black}]
            \node[fill=uofgcobalt] (a) {};
            \node (a1) [below left of = a] {};
            \node (b1) [below right of = a] {};
            \node (aa) [below right of = a1] {};
...
\begin{figure}
    ...
    \begin{tikzpicture}[every node/.style={circle, inner sep=0.5mm}]
        \node[fill=black] (p) {};
        \node[fill=black] (b) [above of = p,node distance=10mm] {};
        \node[fill=black] (a) [left of = b] {};
        \node[fill=black] (c) [right of = b] {};
        \node[fill=black] (d) [above of = b,node distance=10mm] {};
        ...
        \node[fill=black] (a2) [below of = a1] {};
        \node[fill=black] (b2) [right of = a2] {};
        \node[fill=black] (c2) [right of = b2] {};
        \node[fill=gray!50] (a3) [below of = a2] {};
    ...
    \end{tikzpicture}\hspace{0.5cm}\begin{tikzpicture}[every node/.style={circle, inner sep=0.5mm}]
        \node[fill=black] (p) {};
        \node[draw=none] (b) [above of = p,node distance=10mm] {};
        \node[fill=black] (a) [left of = b] {};
        \node[fill=black] (c) [right of = b] {};
        \node[fill=black] (d) [above of = b,node distance=10mm] {};
        ...
        \path (p) edge (a) edge (c)
            ...
            (c) edge (d);
    \end{tikzpicture}
    \caption{$P\rightarrowtail T$, $P\lessnonind{1} U$}
...
\Cref{fig:klessexample} illustrates the three cases possible when filtering by neighbourhood degree
...
isomorphism between every supplemental graph pair. We use paths of length 2, looking at counts of 1,
2, and 3 in the target graph (and so counts of $1 + k$ up to $3 + k$ in the pattern).  We then
...
\nl \Begin{
    \nl \While{\KwSty{true}}{
        \nl \If{\textnormal{no effectively-unit domains (treating all wildcard~values as a single
            value) remain\label{line:effect}}}{
...
We bring these techniques into a full algorithm, listed as \cref{algorithm:sip}. This is derived
from a subgraph isomorphism due to \citet{DBLP:conf/cp/McCreeshP15}, which in a recent evaluation of
algorithm portfolios \citep{thelionpaper} was the single strongest solver. As well as the
modifications to solve $k$-less and induced problems, we have made two simplifications: we do not
use conflict-directed backjumping, and we do not use iterated label filtering to recalculate
neighbourhood degree sequences if vertices are not present in any target domain after
construction---in preliminary experiments, these techniques were not beneficial for $k > 0$. We also
do not strip isolated vertices, as this is not a valid simplification in the induced case.
...
Let $P_{2}$ be the set of all paths of length two between $p$ and $q$, $P_{2} = \{((p,x),(x,q)) : (p,x),(x,q)\in E(P) \}$. As we are looking at paths of length 2, the end vertex on the
\begin{align*}
 \\
