...
auto main(int argc, char * argv[]) -> int
{
    auto subgraph_isomorphism_algorithms = {
        std::make_pair( std::string{ "cilk" },        parallel_subgraph_isomorphism ),
        std::make_pair( std::string{ "cilkix" },      parallel_ix_subgraph_isomorphism )
    };
    ...
    try {
        ...
        display_options.add_options()
            ...
            ("induced",                               "Induced")
            ("parallel-for",                          "Use Cilk parallel for instead of spawn")
            ;
