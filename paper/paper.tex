...
\section{Relationship between non-induced and induced subgraph isomorphism}

The \emph{complement} of a graph $P$ is the graph $\bar{P}~$.  Adjacent vertices in $P$ are non-adjacent in $\bar{P}$ while non-adjacent vertices in $P$ are adjacent in $\bar{P}$.

\begin{proposition}\label{prop:comp}
Let $i$ be an assignment of $T$ vertices to $P$ vertices.  Then $i$ satisfies the definition of $P \hookrightarrow T$ if and only if both $i$ satisfies the definition of $P \rightarrowtail T$ and $\bar{P} \rightarrowtail \bar{T}$.
\end{proposition}

\begin{proof}
Follows directly from the definition of an induced subgraph isomorphism and the definition of the complement of a graph.  Could flesh this out if we need to fill up space and look cleverer??
\end{proof}


\begin{corollary}
Let $i$ be an assignment of $T$ vertices to $P$ vertices.  Then $i$ satisfies the definition of $P \lessind{k} T$ if and only if both $i$ satisfies the definition of $P \lessnonind{k} T$ and $\bar{P} \lessnonind{k} \bar{T}$.
\end{corollary}

Should we just give the corollary as the proof??

In our algorithm we make use of \cref{prop:comp} by seeking an $i$ which simultaneously satisfies $P \rightarrowtail T$ and $\bar{P} \rightarrowtail \bar{T}$ when searching for an induced subgraph isomorphism.  Further, \cref{prop:comp} tells us that every induced subgraph isomorphism is equivalent to simultaneous non-induced subgraph isomorphisms both $P$ and $T$ as well as their complements.
