...
\begin{proposition}
    For a graph $G$, let $G^{n, \ell}$ be the graph with vertex set $\operatorname{V}(G)$. Let $p$ and $q$ be adjacent vertices of $G^{n, \ell}$, if there are at least $n$ simple paths of length
    exactly $\ell$ between $p$ and $q$ in $G$. Then any subgraph-except-$k$ isomorphism
